...
Tweeter.prototype.getAccessToken = function(fn, opt) {
    var self = this,
        fn = fn||noop;

    self.log(self.logLevels.DBUG, 'getAccessToken()');

    return self.get('/oauth/access_token', opt, function(err,data) {
        ...
        if(data) {
            ...
            if(response) {
                ...
                self.config.accessTokenSecret = response.oauth_token_secret;
            }
        }
        fn.call(self);        
    });
